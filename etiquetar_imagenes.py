...
class ventanaEtiquetar:
    ...
    def guardar_metadata (self,filename,tags,desc,perfil_act):
        '''Si existe el csv le agrega la nueva informacion, si no existe lo crea'''
        current_dir = os.path.abspath(__file__) 

        relative_path = "C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\datos\\metadata.csv"
        ...
        while True:
            ...
            if event == '-FOLDER-':                        
                ...
                try:
                    ...
                    pass       
            if event == '-GUARDAR-':
                self.guardar_metadata(filename,tags,desc,perfil_act)
                sg.popup_no_border('Se guardo correctamente la metadata de la imagen')
                with open('C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\logs.csv', 'w', encoding ='UTF-8') as logs:
                    c = csv.writer(logs)
                    c.writerow([perfil_act,datetime.datetime.now(),'modifico una imagen'])
