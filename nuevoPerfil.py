...
class NuevoPerfil:
    ...
    def __init__(self):
        ...
        relative_path = 'C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\icons/'
    ...
    def mostrar_imagen(self,imagen,event):
        ...
        if not event == sg.WINDOW_CLOSED:
            ...
            pass
    '''compruebo que ningun valor este vacio'''
        ...
        while True:
            ...
            if event == 'guardar':
                ...
                if self.validar_campos(values):
                    ...
                    '''guardo los perfiles en la variable perfil si es que existe'''
                    if os.path.exists('C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\datos\\perfil.json'):
                        with open('C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\datos\\perfil.json', "r") as f:
                            perfiles = json.load(f)
                        '''sino la creo'''
                    else:
                        perfiles = []
                    '''agrego el nuevo perfil'''
                    perfiles.append(data)
                    '''actualizo el archivo json'''
                    with open('C:\\Users\\marti\\Desktop\\proyecto\\ventanas\\datos\\perfil.json', "w") as f:
                        json.dump(perfiles, f, indent=4)
                    '''cierro la ventana y voy a la ventana inicio'''
                    self.window.close()
                    inicio = inicio_ventana.VentanaPrincipal()
                    inicio.iniciar_ventana()
            elif event == 'volver': 
                '''vuelvo a la ventana inicio'''
                self.window.close()
                inicio = inicio_ventana.VentanaPrincipal()
                inicio.iniciar_ventana()
            elif event == "-ant-":
                '''voy a la imagen con el indice anterior'''
                self.indice = (self.indice - 1) % len(self.lista_imagen)
                self.mostrar_imagen(self.lista_imagen,event)
            elif event == "-sig-":
                '''voy a la imagen con el indice siguiente'''
                self.indice = (self.indice + 1) % len(self.lista_imagen)
                self.mostrar_imagen(self.lista_imagen,event)
        self.window.close()